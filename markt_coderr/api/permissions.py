...
def _infer_profile_type(user):
    """Infer profile type from user identifiers."""
...
def _get_profile_type(user):
    """Get profile type with fallback inference."""
    try:
...
class IsBusinessUser(permissions.BasePermission):
    """Allow access only for business users."""
...
class IsCustomerUser(permissions.BasePermission):
    """Allow access only for customer users."""
    def has_permission(self, request, view):
        if not request.user or not request.user.is_authenticated:
            return False
...
class IsOfferOwner(permissions.BasePermission):
    """Allow access only for offer owners."""
    def has_object_permission(self, request, view, obj):
        return obj.user_id == request.user.id


class IsReviewOwner(permissions.BasePermission):
    """Allow access only for review owners."""
    def has_object_permission(self, request, view, obj):
        return obj.reviewer_id == request.user.id


class IsOrderBusinessOwner(permissions.BasePermission):
    """Allow access only for the business user of an order."""
    def has_object_permission(self, request, view, obj):
        return obj.business_user_id == request.user.id


class IsProfileOwner(permissions.BasePermission):
    """Allow access only for profile owners."""
    def has_object_permission(self, request, view, obj):
        return obj.user_id == request.user.id


class IsStaffUser(permissions.BasePermission):
    """Allow access only for staff users."""
    def has_permission(self, request, view):
